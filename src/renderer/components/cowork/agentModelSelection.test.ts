...
const models: Model[] = [
  ...
  { id: 'kimi-k2.6', name: 'Kimi K2.6 Server', providerKey: 'qoowork-server', isServerModel: true },
];
...
describe('resolveAgentModelSelection', () => {
  ...
  test('resolves same-id server session model to the server model', () => {
    const result = resolveAgentModelSelection({
      sessionModel: 'qoowork-server/kimi-k2.6',
    ...
    expect(result.selectedModel?.providerKey).toBe('qoowork-server');
