...
const WIN_INBOUND = String.raw`C:\Users\yangwn\AppData\Roaming\qoowork\openclaw\state\media\inbound`;
const MAC_INBOUND = '/Users/yangwn/Library/Application Support/qoowork/openclaw/state/media/inbound';
...
describe('qoowork goal mode command display', () => {
...
describe('Pattern A: NIM/DingTalk', () => {
  test('[图片] with URL and [附件信息] → strip metadata, preserve URL as text', () => {
    ...
    const input = [
      '[图片] https://nos.qoobot.com/xxx.jpg',
      '',
      '[附件信息]',
    ...
    expect(result).toBe('https://nos.qoobot.com/xxx.jpg');
  ...
  test('user text + [图片] → preserve user text and URL', () => {
    ...
    expect(result).toContain('https://nos.qoobot.com/xxx.jpg');
    expect(result).not.toContain('[图片]');
  });
  ...
  test('[文件] with URL → preserve URL', () => {
    const input = '[文件] https://nos.qoobot.com/file.pdf';
    const result = parseUserMessageForDisplay(input);
    expect(result).toBe('https://nos.qoobot.com/file.pdf');
  });
