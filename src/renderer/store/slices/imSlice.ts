...
const imSlice = createSlice({
  ...
  reducers: {
    ...
    setQoobotBeeChanConfig: (state, action: PayloadAction<Partial<QoobotBeeChanConfig>>) => {
      state.config['qoobot-bee'] = { ...state.config['qoobot-bee'], ...action.payload };
    },
