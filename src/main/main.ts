...
import { AppUpdateCoordinator, INSTALLATION_UUID_KEY } from './libs/update/appUpdateCoordinator';
import { AuthCallbackRouter } from './libs/auth/authCallbackRouter';
import {
  appendCallbackReturnTo,
  appendLoginParams,
  startAuthLocalCallback,
} from './libs/auth/authLocalCallbackServer';
import {
...
import { migrateAgentModelRefs, parsePrimaryModelRef, resolveQualifiedAgentModelRef } from './libs/openclaw/openclawAgentModels';
import {
...
} from './libs/openclaw/openclawConfigImpact';
...
} from './libs/openclaw/openclawGatewayRepair';
import {
  addMemoryEntry,
  deleteMemoryEntry,
  ...
  updateMemoryEntry,
  writeBootstrapFile,
} from './libs/openclaw/openclawMemoryFile';
import { collectReferencedEnvVarNames, pickReferencedSecretEnvVars } from './libs/openclaw/openclawSecretEnv';
import { startOpenClawTokenProxy, stopOpenClawTokenProxy } from './libs/openclaw/openclawTokenProxy';
import { migrateMainAgentWorkspace } from './libs/openclaw/openclawWorkspaceMigration';
...
import {
  ...
  LOCAL_WEB_SERVICE_PORTS,
  LOCAL_WEB_SERVICE_PROBE_TIMEOUT_MS,
  LOCAL_WEB_SERVICE_TITLE_MAX_LENGTH,
  MAX_ARTIFACT_SHARE_CONTENT_CHARS,
  MAX_INLINE_ATTACHMENT_BYTES,
  MAX_MEMORY_USER_MEMORIES_MAX_ITEMS,
  MIME_EXTENSION_MAP,
  MIN_MEMORY_USER_MEMORIES_MAX_ITEMS,
  OPENCLAW_DAILY_LOG_RE,
  OPENCLAW_DAILY_LOG_RETENTION_DAYS,
  PowerSaveBlockerType,
  SHARE_DEPLOYMENT_CANDIDATE_SOURCES,
  SHARE_DEPLOYMENT_PROJECT_CANDIDATE_MAX_ITEMS,
} from './setup/constants';
import type {
  HtmlShareCreateFromArtifactFileInput,
  HtmlShareCreateFromHtmlFileInput,
  HtmlShareGetByArtifactFileInput,
  HtmlShareGetByHtmlFileInput,
  HtmlShareUpdateAccessModeInput,
  HtmlShareUpdateFromArtifactFileInput,
  HtmlShareUpdateFromHtmlFileInput,
  HtmlShareUpdateStatusInput,
...
const openClawConfigHasServerModels = (modelIds: string[]): boolean => {
  ...
  try {
    ...
    const serverProviderModels = parsed.models?.providers?.[OpenClawProviderId.qooworkServer]?.models;
...
const resolveInlineAttachmentDir = (cwd?: string): string => {
  ...
  return path.join(app.getPath('temp'), 'qoowork', 'attachments');
};
...
const buildLogExportFileName = (): string => {
  ...
  return `qoowork-logs-${datePart}-${timePart}.zip`;
};
...
const disableGpu =
  process.env.qoowork_DISABLE_GPU === '1' ||
  process.env.qoowork_DISABLE_GPU === 'true' ||
...
const isqooworkServerModelRef = (modelRef: string): boolean => {
  const normalized = modelRef.trim();
  if (!normalized) return false;

  const parsed = parsePrimaryModelRef(normalized);
  if (parsed) {
    return parsed.providerId === ProviderName.qooworkServer;
  }
...
const shouldRefreshServerQuotaForSession = (sessionId: string): boolean => {
  ...
  if (sessionModelRef) {
    return isqooworkServerModelRef(sessionModelRef);
  }
  ...
  const agentModelRef = session?.agentId
    ...
    : '';
  if (agentModelRef) {
    return isqooworkServerModelRef(agentModelRef);
  }

  const apiConfig = resolveCurrentApiConfig();
  return apiConfig.providerMetadata?.providerName === ProviderName.qooworkServer;
};
...
const getOpenClawConfigSync = (): OpenClawConfigSync => {
  if (!openClawConfigSync) {
    openClawConfigSync = new OpenClawConfigSync({
      ...
      getQoobotBeeChanConfig: () => {
        try {
          return getIMGatewayManager().getConfig()['qoobot-bee'];
        } catch {
          return null;
        }
      },
...
// 获取正确的预加载脚本路径 (preload.js 与 main.js 在同一目录)
...
if (!gotTheLock) {
  ...
  if (!app.isPackaged) {
    ...
    app.setAsDefaultProtocolClient('qoowork', process.execPath, [
      path.resolve(process.argv[1]),
    ]);
  } else {
    app.setAsDefaultProtocolClient('qoowork');
  }
  ...
  /**
   * Parse a qoowork:// deep link and send (or buffer) the auth code.
   */
  ...
  app.on('second-instance', (_event, commandLine, workingDirectory) => {
    ...
    const deepLink = commandLine.find(arg => arg.startsWith('qoowork://'));
    if (deepLink) {
            ...
            ? [
                {
                  ...
                  filePath: path.join(app.getPath('appData'), 'qoowork', 'install-timing.log'),
                },
              ]
            : []),
        ],
  ...
  ipcMain.handle(DataMigrationIpc.Backup, async event => {
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to back up qoowork data',
      };
    }
  });
  ...
  ipcMain.handle(DataMigrationIpc.Restore, async event => {
    ...
    try {
      ...
      return {
        ...
        error: success ? undefined : restoreResult?.error || 'Failed to import qoowork data backup',
      };
    } catch (error) {
      isCleanupInProgress = false;
      const message = error instanceof Error ? error.message : 'Failed to import qoowork data backup';
  ...
  ipcMain.handle('github-copilot:request-device-code', async () => {
    const { requestDeviceCode } = await import('./libs/auth/githubCopilotAuth');
    try {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const { pollForAccessToken, getCopilotToken, getGitHubUser } =
        await import('./libs/auth/githubCopilotAuth');
      try {
  ...
  ipcMain.handle('github-copilot:cancel-polling', async () => {
    const { cancelPolling } = await import('./libs/auth/githubCopilotAuth');
    cancelPolling();
  });
  ...
  ipcMain.handle('openai-codex-oauth:start', async () => {
    const { startOpenAICodexLogin } = await import('./libs/auth/openaiCodexAuth');
    try {
  ...
  ipcMain.handle('openai-codex-oauth:cancel', async () => {
    const { cancelOpenAICodexLogin } = await import('./libs/auth/openaiCodexAuth');
    cancelOpenAICodexLogin();
  });

  ipcMain.handle('openai-codex-oauth:logout', async () => {
    const { logoutOpenAICodex } = await import('./libs/auth/openaiCodexAuth');
    logoutOpenAICodex();
  });

  ipcMain.handle('openai-codex-oauth:status', async () => {
    const { readOpenAICodexAuthFile } = await import('./libs/auth/openaiCodexAuth');
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const tmpDir = path.join(app.getPath('temp'), 'qoowork-thumbnails');
  ...
  ipcMain.handle(ShellIpc.OpenHtmlInBrowser, async (_event, htmlContent: string) => {
    try {
      const tmpDir = path.join(os.tmpdir(), 'qoowork-preview');
...
<body>
  ...
  const initApp = async () => {
    ...
    const defaultProjectDir = path.join(os.homedir(), 'qoowork', 'project');
    ...
    // Inject auth getters for qoowork-server provider routing
    ...
    if (storedGithubToken) {
      import('./libs/auth/githubCopilotAuth')
    ...
    registerProxyTokenRefresher('qoowork-server', async () => {
      const tokens = getAuthTokens();
      if (!tokens?.refreshToken) return null;
      const serverBaseUrl = getServerApiBaseUrl();
      try {
    ...
    registerProxyTokenRefresher('github-copilot', async () => {
      try {
        const { refreshCopilotTokenNow } = await import('./libs/auth/copilotTokenManager');
    ...
    // qoowork-server provider can use the proxy URL in its config.
    ...
    // can match qoowork-server models without falling back.
    ...
    const coldStartDeepLink = process.argv.find(arg => arg.startsWith('qoowork://'));
