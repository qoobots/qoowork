 ...
 * and Telegram, Discord, QQ, WeCom, Weixin, POPO, QoobotBee via OpenClaw
 */
...
export class IMGatewayManager extends EventEmitter {
  ...
  private setupGatewayEventForwarding(): void {
    ...
    // qoobot-bee runs via OpenClaw; no direct gateway events to forward
  ...
  reconnectAllDisconnected(): void {
    ...
    // qoobot-bee runs via OpenClaw; no direct reconnect needed
  ...
  setConfig(
    ...
    // Hot-update qoobot-bee config: sync OpenClaw config when credentials change.
    // Only perform sync when syncGateway is explicitly true (i.e. user clicked Save).
    if (options?.syncGateway && config['qoobot-bee']) {
      const oldNb = previousConfig['qoobot-bee'];
      const newNb = { ...oldNb, ...config['qoobot-bee'] };
      const credentialsChanged =
        newNb.clientId !== oldNb?.clientId ||
        newNb.secret !== oldNb?.secret;
      if (credentialsChanged) {
        console.log('[IMGatewayManager] qoobot-bee credentials changed, syncing OpenClaw config...');
        this.syncOpenClawConfig?.('im-config-change:qoobot-bee', {
  ...
  getStatus(): IMGatewayStatus {
    ...
    return {
      ...
      'qoobot-bee': (() => {
        const beeConfig = config['qoobot-bee'];
        return {
  ...
  async testGateway(
    ...
    // QooBot Bee is an internal relay channel with no standalone gateway to test
    if (platform === 'qoobot-bee') {
      return {
        platform,
        testedAt: Date.now(),
        verdict: 'warn',
        checks: [{
          ...
          level: 'info',
          message: 'QooBot Bee channel does not support standalone connectivity testing.',
        }],
      };
    }
  ...
  async startGateway(platform: Platform): Promise<void> {
    ...
    if (platform === 'dingtalk') {
      ...
      await this.ensureOpenClawGatewayConnected?.();
      return;
    } else if (platform === 'qoobot-bee') {
      // qoobot-bee runs via OpenClaw gateway
      console.log('[IMGatewayManager] qoobot-bee in OpenClaw mode, syncing config instead of starting direct gateway');
      await this.syncOpenClawConfig?.('im-gateway-start:qoobot-bee');
      await this.ensureOpenClawGatewayConnected?.();
      return;
    } else if (platform === 'qq') {
  ...
  async stopGateway(platform: Platform): Promise<void> {
    if (platform === 'dingtalk') {
      ...
      // qoobot-bee runs via OpenClaw gateway
      console.log('[IMGatewayManager] qoobot-bee in OpenClaw mode, syncing disabled config');
      await this.syncOpenClawConfig?.('im-gateway-stop:qoobot-bee');
      return;
    } else if (platform === 'qq') {
  ...
  async startAllEnabled(): Promise<void> {
    ...
    if (config['qoobot-bee']?.enabled && config['qoobot-bee']?.clientId && config['qoobot-bee']?.secret) {
      openClawPlatformsToStart.push('qoobot-bee');
    }
  ...
  isConnected(platform: Platform): boolean {
    ...
    if (platform === 'qoobot-bee') {
      // qoobot-bee runs via OpenClaw; status comes from OpenClaw
      const config = this.getConfig();
      return Boolean(config['qoobot-bee']?.enabled && config['qoobot-bee']?.clientId && config['qoobot-bee']?.secret);
    }
    if (platform === 'qq') {
  ...
  async sendNotification(platform: Platform, _text: string): Promise<boolean> {
    ...
    try {
      if (platform === 'nim') {
        ...
        // qoobot-bee runs via OpenClaw; notifications not yet supported
        console.log('[IMGatewayManager] qoobot-bee notification via OpenClaw not yet supported');
      }
      return true;
    } catch (error: any) {
  ...
  private static readonly POPO_QRCODE_BASE_URL =
    'https://f2e.popo.qoobot.com/polymers/qoowork-bot-h5/?pp_htb=1&pp_back_type=cross&taskToken=';
  private static readonly POPO_POLLING_API =
    'https://open.popo.qoobot.com/open-apis/no-auth/openclaw/v1/polling';
  private static readonly POPO_COMPLETE_API =
    'https://open.popo.qoobot.com/open-apis/no-auth/openclaw/v1/completed';
  ...
  private buildMergedConfig(configOverride?: Partial<IMGatewayConfig>): IMGatewayConfig {
    ...
    return {
      ...
      'qoobot-bee': { ...current['qoobot-bee'], ...(configOverride['qoobot-bee'] || {}) },
  ...
  private getMissingCredentials(platform: Platform, config: IMGatewayConfig): string[] {
    ...
    if (platform === 'qoobot-bee') {
      const fields: string[] = [];
      if (!config['qoobot-bee']?.clientId) fields.push('clientId');
      if (!config['qoobot-bee']?.secret) fields.push('secret');
      return fields;
    }
    if (platform === 'qq') {
      const qqInstances = config.qq?.instances || [];
  ...
  private async runAuthProbe(platform: Platform, config: IMGatewayConfig): Promise<string> {
    ...
    if (platform === 'qoobot-bee') {
      const nbConfig = config['qoobot-bee'];
      const clientId = nbConfig?.clientId;
      const secret = nbConfig?.secret;
      if (!clientId || !secret) {
        throw new Error(t('imConfigIncomplete'));
      }
      return t('imQoobotBeeConfigReady', { clientId });
    }
  ...
  private getStartedAtMs(platform: Platform, status: IMGatewayStatus): number | null {
    ...
    if (platform === 'qoobot-bee') return status['qoobot-bee'].startedAt;
  ...
  private getLastInboundAt(platform: Platform, status: IMGatewayStatus): number | null {
    ...
    if (platform === 'qoobot-bee') return status['qoobot-bee'].lastInboundAt;
  ...
  private getLastOutboundAt(platform: Platform, status: IMGatewayStatus): number | null {
    ...
    if (platform === 'qoobot-bee') return status['qoobot-bee'].lastOutboundAt;
  ...
  private getLastError(platform: Platform, status: IMGatewayStatus): string | null {
    ...
    if (platform === 'qoobot-bee') return status['qoobot-bee'].lastError;
