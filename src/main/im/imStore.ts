...
export class IMStore {
  ...
  private migrateDefaults(): void {
    ...
    // Migrate 'xiaomifeng' config key to 'qoobot-bee'
    const oldXmfRow = this.db
      .prepare('SELECT value FROM im_config WHERE key = ?')
      .get('xiaomifeng') as { value: string } | undefined;
    const newBeeRow = this.db
      .prepare('SELECT value FROM im_config WHERE key = ?')
      .get('qoobot-bee') as { value: string } | undefined;
    if (oldXmfRow && !newBeeRow) {
      try {
        const oldConfig = JSON.parse(oldXmfRow.value) as Partial<QoobotBeeChanConfig>;
        const now = Date.now();
        this.db
          .prepare('INSERT INTO im_config (key, value, updated_at) VALUES (?, ?, ?)')
          .run('qoobot-bee', JSON.stringify({ ...DEFAULT_QOOBOT_BEE_CONFIG, ...oldConfig }), now);
        this.db.prepare('DELETE FROM im_config WHERE key = ?').run('xiaomifeng');
        console.log('[IMStore] Migrated xiaomifeng config to qoobot-bee');
      } catch {
        // Ignore parse errors
      }
    }
  ...
  getConfig(): IMGatewayConfig {
    ...
    const qoobotBeeChan =
      this.getConfigValue<QoobotBeeChanConfig>('qoobot-bee') ?? DEFAULT_QOOBOT_BEE_CONFIG;
    ...
    return {
      ...
      nim: nimMulti,
      'qoobot-bee': resolveEnabled(qoobotBeeChan, DEFAULT_QOOBOT_BEE_CONFIG),
      qq: qqMulti,
      wecom: wecomMulti,
      popo: popoMulti,
  ...
  setConfig(config: Partial<IMGatewayConfig>): void {
    ...
    if (config['qoobot-bee']) {
      this.setQoobotBeeChanConfig(config['qoobot-bee']);
    }
    if (config.qq) {
  ...
  // ==================== QoobotBee Chan Config ====================

  getQoobotBeeChanConfig(): QoobotBeeChanConfig {
    const stored = this.getConfigValue<QoobotBeeChanConfig>('qoobot-bee');
    return { ...DEFAULT_QOOBOT_BEE_CONFIG, ...stored };
  }

  setQoobotBeeChanConfig(config: Partial<QoobotBeeChanConfig>): void {
    const current = this.getQoobotBeeChanConfig();
    this.setConfigValue('qoobot-bee', { ...current, ...config });
  }
  ...
  isConfigured(): boolean {
    ...
    const hasQoobotBeeChan = !!(config['qoobot-bee']?.clientId && config['qoobot-bee']?.secret);
