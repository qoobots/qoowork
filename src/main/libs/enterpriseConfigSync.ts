 ...
 * (e.g., 'deepseek', 'anthropic') instead of the generic 'qoowork'.
 */
function syncModelConfig(configPath: string, store: SqliteStore): void {
  ...
  try {
    ...
    for (const [providerId, providerConfig] of Object.entries(models.providers)) {
      ...
      // Resolve apiKey: use plain text value, skip placeholders like ${QOOWORK_...}
...
function syncIMChannels(configPath: string, imStore: IMStore): void {
  ...
  try {
    ...
    const PLATFORM_SETTERS: Record<string, (cfg: any) => void> = {
      ...
      'qoobot-bee': (cfg) => imStore.setQoobotBeeChanConfig(cfg),
    };
...
function syncOpenClawAgentList(
  ...
  try {
    ...
    if (syncedCount > 0) {
      console.log(`[Enterprise] synced ${syncedCount} agent config(s) to Qoowork agents`);
    }
  } catch (error) {
