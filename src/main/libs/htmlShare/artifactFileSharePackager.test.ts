...
async function createTempRoot(): Promise<string> {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'qoowork-artifact-share-packager-test-'));
  tempRoots.push(root);
  return root;
}
...
describe('artifactFileSharePackager', () => {
  ...
  test('packages Markdown with same-directory local images and omits remote images', async () => {
    ...
    const manifest = JSON.parse(await zip.file('_QOOWORK_share_manifest.json')!.async('string'));

    expect(entries).toHaveLength(3);
    expect(entries).toContain('README.md');
    expect(entries).toContain('_QOOWORK_share_manifest.json');
    expect(entries.some(entry => entry.startsWith('_QOOWORK_assets/') && entry.endsWith('arch.png'))).toBe(true);
    expect(markdown).toContain('_QOOWORK_assets/');
  ...
  test('packages Markdown file URL images inside the Markdown directory', async () => {
    ...
    const manifestText = await zip.file('_QOOWORK_share_manifest.json')!.async('string');
    const manifest = JSON.parse(manifestText);

    expect(entries).toHaveLength(3);
    expect(entries.some(entry =>
      entry.startsWith('_QOOWORK_assets/') &&
  ...
  test('removes the temporary Markdown archive when packaging fails', async () => {
    ...
    const mkdtempSpy = vi.spyOn(fs.promises, 'mkdtemp').mockImplementation(async (prefix, options) => {
      ...
      if (prefix.includes('qoowork-artifact-share-')) packagingTempDir = created;
      return created;
    });
