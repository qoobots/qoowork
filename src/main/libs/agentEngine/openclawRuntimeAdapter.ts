...
import { extractOpenClawAssistantStreamParts,extractOpenClawAssistantStreamText } from '../openclaw/openclawAssistantText';
import {
...
} from '../openclaw/openclawHistory';
import { buildOpenClawLocalTimeContextPrompt } from '../openclaw/openclawLocalTimeContextPrompt';
import { consumeRecentOpenClawTokenProxyQuotaError } from '../openclaw/openclawTokenProxy';
import {
...
const buildMediaGenerationTurnInstruction = (selection?: CoworkMediaSelection, hasMediaSkillActive?: boolean): string => {
  if (!selection || selection.mode === 'none') {
    if (hasMediaSkillActive) {
      return [
        '[qoowork media generation tools — NOT AVAILABLE]',
        'The qoowork_image_generate and qoowork_video_generate tools are NOT available for this turn.',
        'Do NOT call qoowork_image_generate or qoowork_video_generate.',
  ...
  const lines = [
    '[qoowork media generation turn instruction]',
    'The user selected a qoowork media generation model for this turn.',
    'IMPORTANT: Do NOT read or use the "seedance" or "seedream" skills for this request.',
    'The qoowork media generation tools (qoowork_image_generate / qoowork_video_generate) replace those skills when a media model is selected.',
    'Do not run any skill scripts for image or video generation. Use only the qoowork_* tools specified below.',
  ];

  if (selection.mode === 'image') {
    lines.push('If the current user request asks to create, generate, draw, render, or make an image/photo/picture, you must call the qoowork_image_generate tool exactly once with action="generate".');
    ...
    lines.push('If the current user request asks to create, generate, render, or make a video, you must call the qoowork_video_generate tool exactly once with action="generate".');
    ...
    lines.push('If the current user request asks for image generation, call qoowork_image_generate with action="generate".');
    lines.push('If the current user request asks for video generation, call qoowork_video_generate with action="generate".');
  ...
  if (!selection.imageModelId && !selection.videoModelId && selection.modelId?.trim()) {
    lines.push(`You MUST use model "${selection.modelId.trim()}" for media generation. Do NOT use a different model unless the user explicitly requests a different qoowork media model by name.`);
  }
...
const buildMediaReferencePromptSection = (mediaReferences?: CoworkMediaAttachmentRef[]): string => {
  ...
  const lines = [
    '[qoowork media reference mapping]',
    ...
    'When calling qoowork_image_generate or qoowork_video_generate, pass mapped file paths or URLs as tool arguments. Do not pass @ media tokens as image, images, firstFrame, lastFrame, referenceImages, media.url, video, or videos values.',
    'For qoowork_image_generate, prefer image with the mapped path for one referenced image and images for multiple referenced images.',
  ];
  ...
  for (const ref of refs) {
    ...
    const locations = [
      ...
      !ref.localPath && !ref.remoteUrl && ref.dataUrl ? 'dataUrl fallback available through qoowork host' : '',
    ].filter(Boolean);
...
export class OpenClawRuntimeAdapter extends EventEmitter implements CoworkRuntime {
   ...
   * so qoowork can recover even when the gateway fails to deliver the abort event.
   */
   ...
   * for a qoowork-initiated session.
   */
  ...
  private buildSystemPromptPrefix(systemPrompt: string): string {
    return [
      '[qoowork system instructions]',
      'Apply the instructions below as the highest-priority guidance for this session.',
      'If earlier qoowork system instructions exist, replace them with this version.',
      systemPrompt,
    ].join('\n');
  }
  ...
  private buildBridgePrefix(messages: CoworkMessage[], currentPrompt: string): string {
    ...
    const sections = [
      '[Context bridge from previous qoowork conversation]',
  ...
  private handleAgentEvent(payload: unknown, seq?: number): void {
    ...
    if (sessionId && !this.activeTurns.has(sessionId) && sessionKey && stream !== 'error' && !this.terminatedRunIds.has(runId)) {
      // Desktop sessions (qoowork:*) that were manually stopped must not be
   ...
   * gateway rather than the qoowork UI.
   *
  ...
  private ensureActiveTurn(sessionId: string, sessionKey: string, runId: string): void {
    ...
    // happens to clear it.
    // Only clear for channel/cron sessions.  Desktop sessions (qoowork:*)
