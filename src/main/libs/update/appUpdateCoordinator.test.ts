...
function seedReadyFile(store: SqliteStore, updatesDir: string, source: AppUpdateSource): string {
  ...
  const filePath = path.join(updatesDir, `qoowork-update-${source}-1.exe`);
  ...
  store.set(readyFileStoreKey(source), {
    ...
    info: {
      ...
      url: `https://updates.example.com/qoowork-${READY_VERSION}.exe`,
    },
  });
  return filePath;
}
...
describe('AppUpdateCoordinator', () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qoowork-update-test-'));
      ...
      json: async () => ({
        ...
        data: {
          value: {
            ...
            macIntel: { url: `https://updates.example.com/qoowork-${READY_VERSION}.dmg` },
            macArm: { url: `https://updates.example.com/qoowork-${READY_VERSION}.dmg` },
            windowsX64: { url: `https://updates.example.com/qoowork-${READY_VERSION}.exe` },
          },
        },
      }),
    });
