...
async function handleRequest(
  ...
  if (method === 'POST' && (url.pathname === '/v1/copilot/chat/completions' || url.pathname === '/copilot/chat/completions')) {
    ...
    const { getCurrentCopilotToken, refreshCopilotTokenNow } = await import('./auth/copilotTokenManager');
  ...
  // the qoowork provider. Transparently proxy these requests to the upstream with
  ...
  // Inject session_id and user_message for qoowork-server logging only.
  // Strict providers (e.g. Gemini) reject unknown payload fields.
  if (upstreamConfig.provider === 'qoowork-server') {
