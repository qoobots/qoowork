...
describe('OpenClawConfigSync runtime config output', () => {
  ...
  test('writes OpenClaw config fields required by qoowork patches', async () => {
    ...
    const result = sync.sync('qoowork-patch-dependent-fields');
  ...
  test('merges all server models into existing qoowork provider and updates image input', async () => {
    ...
    mockRuntimeState.rawApiConfig = {
      config: {
        baseURL: 'https://qoowork-server.qoobot.com/api/proxy/v1',
        apiKey: 'access-token',
        model: 'qwen3.5-plus-YoudaoInner',
        apiType: 'openai',
      },
      providerMetadata: {
        providerName: 'qoowork-server',
        codingPlanEnabled: false,
        supportsImage: false,
    ...
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    const provider = config.models.providers['qoowork-server'];
    expect(provider.baseUrl).toBe('http://127.0.0.1:56646/v1');
    expect(provider.apiKey).toBe('${QOOWORK_PROXY_TOKEN}');
    expect(JSON.stringify(config)).not.toContain('QOOWORK_APIKEY_SERVER');
    ...
    expect(config.agents.defaults.models).toEqual(expect.objectContaining({
      'qoowork-server/qwen3.5-plus-YoudaoInner': {
        params: {
      ...
      'qoowork-server/qwen3.6-plus-YoudaoInner': {
        params: {
          cacheRetention: 'short',
          contextCacheProvider: 'dashscope',
          contextCacheMode: 'explicit',
      ...
      'qoowork-server/claude-sonnet-4-6-YoudaoInner': {
        params: {
          cacheRetention: 'short',
        },
      },
      'qoowork-server/claude-opus-4-YoudaoInner': {
        params: {
          cacheRetention: 'short',
        },
      },
      'qoowork-server/claude-sonnet-4-6': {
        params: {
          cacheRetention: 'short',
  ...
  test('writes Claude OpenAI-compatible explicit cache params when server metadata is not loaded', async () => {
    ...
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    expect(config.models.providers['qoowork-server'].models).toEqual(expect.arrayContaining([
      expect.objectContaining({
        id: 'claude-sonnet-4-6',
        api: 'openai-completions',
      }),
  ...
  test('writes a complete agent model allowlist when any model has custom params', async () => {
    ...
    expect(modelDefaults).toEqual(expect.objectContaining({
      ...
      'qoowork-server/MiniMax-M2.7-YoudaoInner': {},
      'qoowork-server/kimi-k2.6-inhouse-ZhiYun': {},
    }));
  ...
  test('enables media generation plugin when media entitlement is available', async () => {
    ...
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    expect(config.plugins.entries['qoowork-media-generation']).toEqual({
      enabled: true,
      config: {
  ...
  test('maps MiniMax OAuth mode to the MiniMax portal provider', async () => {
    ...
    expect(selection.providerConfig.auth).toBe(AuthType.OAuth);
    expect(selection.providerConfig.apiKey).toBe('${QOOWORK_APIKEY_MINIMAX}');
  ...
  test('writes NIM env vars with the same indexes as enabled channel accounts', async () => {
    ...
    expect(config.channels.nim.accounts['nim-work'].nimToken).toBe(
      'work-app|work-account|${QOOWORK_NIM_TOKEN_1}',
    );

    const env = sync.collectSecretEnvVars();
    expect(env).not.toHaveProperty('QOOWORK_NIM_TOKEN');
    expect(env.QOOWORK_NIM_TOKEN_1).toBe('work-token');
  });
  ...
  test('writes managed browser policy forcing host target', async () => {
    ...
    expect(agentsMd).toContain('qoowork does not support sandbox browser execution in this version.');
  ...
  test('marks MCP server config changes as restart impact', async () => {
    ...
    const sync = await createSync({
      getResolvedMcpServers: () => [{
        ...
        command: 'node',
        args: ['server.js'],
        env: { TAVILY_API_KEY: '${QOOWORK_TAVILY_API_KEY}' },
      }],
    });
