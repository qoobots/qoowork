...
import { getCodexHomeDir } from '../auth/openaiCodexAuth';
...
export function buildOpenClawCompileCacheEnv(compileCacheDir: string): NodeJS.ProcessEnv {
  return {
    ...
    // The cache is already configured by qoowork. Prevent the packaged
...
export class OpenClawEngineManager extends EventEmitter {
  ...
  private async doStartGateway(): Promise<OpenClawEngineStatus> {
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      qoowork_SKILLS_ROOT: skillsRoot,
      ...
      // Disable Bonjour/mDNS LAN discovery advertising.  qoowork is a
      ...
      qoowork_ELECTRON_PATH: electronNodeRuntimePath.replace(/\\/g, '/'),
      qoowork_OPENCLAW_ENTRY: openclawEntry.replace(/\\/g, '/'),
    ...
    // find the qoowork-managed Python instead of the Windows Store stub.
  ...
  private ensureOpenClawWorkerShimsForBundle(runtimeRoot: string): void {
    try {
      ...
      if (result.protectedExisting.length > 0) {
        console.warn(
          `[OpenClaw] Skipped ${result.protectedExisting.length} worker shim(s) because existing files are not qoowork shims.`,
        );
      }
    } catch (error) {
  ...
  private ensureBundledCliShims(): string | null {
    ...
    const shellWrapper = [
      '#!/usr/bin/env bash',
      'if [ -z "${qoowork_OPENCLAW_ENTRY:-}" ]; then',
      '  echo "qoowork_OPENCLAW_ENTRY is not set" >&2',
      '  exit 127',
      'fi',
      'if [ -n "${qoowork_ELECTRON_PATH:-}" ]; then',
      '  exec env ELECTRON_RUN_AS_NODE=1 "${qoowork_ELECTRON_PATH}" "${qoowork_OPENCLAW_ENTRY}" "$@"',
      'fi',
      'if command -v node >/dev/null 2>&1; then',
      '  exec node "${qoowork_OPENCLAW_ENTRY}" "$@"',
      'fi',
      'echo "Neither qoowork_ELECTRON_PATH nor node is available for OpenClaw CLI." >&2',
      'exit 127',
      '',
    ].join('\n');
    const windowsWrapper = [
      '@echo off',
      'if "%qoowork_OPENCLAW_ENTRY%"=="" (',
      '  echo qoowork_OPENCLAW_ENTRY is not set 1>&2',
      '  exit /b 127',
      ')',
      'if not "%qoowork_ELECTRON_PATH%"=="" (',
      '  set ELECTRON_RUN_AS_NODE=1',
      '  "%qoowork_ELECTRON_PATH%" "%qoowork_OPENCLAW_ENTRY%" %*',
      '  exit /b %ERRORLEVEL%',
      ')',
      'node "%qoowork_OPENCLAW_ENTRY%" %*',
      '',
    ].join('\r\n');
