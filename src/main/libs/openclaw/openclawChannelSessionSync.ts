 ...
 * to local Cowork sessions so that conversations are visible in the qoowork UI.
 */
...
const qoowork_SESSION_PREFIX = 'qoowork:';
...
export function buildManagedSessionKey(
  ...
  return `agent:${normalizedAgentId}:qoowork:${normalizedSessionId}`;
}
...
export function parseManagedSessionKey(
  ...
  if (raw.startsWith(qoowork_SESSION_PREFIX)) {
    const sessionId = raw.slice(qoowork_SESSION_PREFIX.length).trim();
  ...
  if (parts.length < 4 || parts[0] !== 'agent' || parts[2] !== 'qoowork') {
    return null;
  }
...
function getChannelTitlePrefix(platform: string): string {
  const i18nMap: Record<string, string> = {
    ...
    'qoobot-bee': t('channelPrefixQoobotBee'),
  };
 ...
 *   - "{accountId}:{peerKind}:{peerId}"      e.g. "bot1:group:12345@popo.qoobot.com"
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip qoowork-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.log('[ChannelSessionSync] skipped: qoowork-originated session');
      return null;
    }
