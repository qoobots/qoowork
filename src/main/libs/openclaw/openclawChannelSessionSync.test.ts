...
test('parseManagedSessionKey handles raw local session keys', () => {
  expect(parseManagedSessionKey('qoowork:abc-123')).toEqual({
    agentId: null,
    sessionId: 'abc-123',
  });
});
...
test('parseManagedSessionKey handles canonical local session keys', () => {
  expect(parseManagedSessionKey('agent:main:qoowork:abc-123')).toEqual({
    agentId: 'main',
    sessionId: 'abc-123',
  });
});
...
test('buildManagedSessionKey emits canonical local session keys', () => {
  ...
  ).toBe(`agent:${DEFAULT_MANAGED_AGENT_ID}:qoowork:abc-123`);
  expect(
    buildManagedSessionKey('abc-123', 'secondary'),
  ).toBe('agent:secondary:qoowork:abc-123');
});

test('parseChannelSessionKey ignores managed local session keys', () => {
  expect(parseChannelSessionKey('qoowork:abc-123')).toBe(null);
  expect(parseChannelSessionKey('agent:main:qoowork:abc-123')).toBe(null);
});

test('channel sync does not treat managed local session keys as channel sessions', () => {
  const sync = createSync();

  expect(isManagedSessionKey('agent:main:qoowork:abc-123')).toBe(true);
  expect(sync.isChannelSessionKey('agent:main:qoowork:abc-123')).toBe(false);
  expect(sync.resolveOrCreateSession('agent:main:qoowork:abc-123')).toBe(null);
  expect(sync.resolveOrCreateMainAgentSession('agent:main:qoowork:abc-123')).toBe(null);
});
...
test('buildChannelDisplayName keeps group prefix', () => {
  expect(buildChannelDisplayName('group:3911967@popo.qoobot.com')).toBe('group:3911967');
});

test('buildChannelDisplayName handles account:direct:peer format', () => {
  expect(buildChannelDisplayName('bot1:direct:zhangsan@corp.qoobot.com')).toBe('zhangsan');
});

test('buildChannelDisplayName handles account:group:peer format', () => {
  expect(buildChannelDisplayName('bot1:group:12345@popo.qoobot.com')).toBe('group:12345');
});
