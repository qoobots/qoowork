 ...
 * - Offline archive support via qoowork_PORTABLE_GIT_ARCHIVE
 * - Mirror URL override via qoowork_PORTABLE_GIT_URL
...
async function resolveArchive(required) {
  const envArchive = resolveInputPath(process.env.qoowork_PORTABLE_GIT_ARCHIVE);
  if (envArchive) {
    if (!isNonEmptyFile(envArchive)) {
      throw new Error(
        `qoowork_PORTABLE_GIT_ARCHIVE points to an invalid file: ${envArchive}`
      );
    }
    console.log(`[setup-mingit] Using local archive from qoowork_PORTABLE_GIT_ARCHIVE: ${envArchive}`);
  ...
  const urlFromEnv = typeof process.env.qoowork_PORTABLE_GIT_URL === 'string'
    ? process.env.qoowork_PORTABLE_GIT_URL.trim()
    : '';
  ...
  try {
    ...
    if (required) {
      throw new Error(
        ...
        + 'Set qoowork_PORTABLE_GIT_ARCHIVE to a local offline package or '
        + 'set qoowork_PORTABLE_GIT_URL to a reachable mirror. '
        + `Original error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
...
async function ensurePortableGit(options = {}) {
  ...
  const shouldRun = process.platform === 'win32' || required || process.env.qoowork_SETUP_MINGIT_FORCE === '1';
