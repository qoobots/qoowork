 ...
 * and waits for the user's response via an HTTP callback to qoowork.
 *
 * This enables delete-confirmation modals on the qoowork desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for qoowork desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool((ctx) => {
      // Only enable for qoowork desktop sessions (sessionKey starts with 'agent:main:qoowork:').
      ...
      const isLocalDesktop = sessionKey.startsWith('agent:main:qoowork:');
      if (!isLocalDesktop) {
        return null;
      }
