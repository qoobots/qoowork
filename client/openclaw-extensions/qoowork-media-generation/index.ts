...
import { isqooworkDesktopSessionKey } from './sessionKey';
  ...
  try {
    const response = await fetch(config.callbackUrl, {
      ...
      headers: {
        ...
        'x-qoowork-media-secret': config.secret,
      },
...
const plugin = {
  id: 'qoowork-media-generation',
  name: 'QooworkMediaGeneration',
  description: 'Image and video generation tools powered by qoowork server.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    if (!config.callbackUrl || !config.secret) {
      api.logger.info('[qoowork-media-generation] skipped: callbackUrl or secret not configured.');
      return;
    }

    api.registerTool((ctx) => {
      const sessionKey = ctx.sessionKey ?? '';
      if (!isqooworkDesktopSessionKey(sessionKey)) {
        return null;
      }

      return {
        name: 'qoowork_image_generate',
        label: 'Image Generation',
        description: [
          'Generate images using qoowork server.',
          'Supports text-to-image and image-to-image generation.',
          'If the system prompt includes a qoowork media reference mapping, use mapped file paths or URLs in image/images arguments and never pass @ media tokens as tool argument values.',
        ...
        async execute(id: string, params: unknown) {
          ...
          try {
            api.logger.info(`[qoowork-media-generation] image tool callback started: toolCallId=${id} args=${JSON.stringify(sanitizeArgsForLog(args))}`);
            const startedAt = Date.now();
            const result = await callMediaBridge(config, {
              tool: 'qoowork_image_generate',
              args,
              context: { sessionKey, toolCallId: id },
            });
            api.logger.info(`[qoowork-media-generation] image tool callback completed: toolCallId=${id} elapsedMs=${Date.now() - startedAt} isError=${result.isError === true}`);
            return result;
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            api.logger.info(`[qoowork-media-generation] image tool callback failed: toolCallId=${id} error=${message}`);
    ...
    api.registerTool((ctx) => {
      ...
      return {
        name: 'qoowork_video_generate',
        label: 'Video Generation',
        description: [
          'Generate videos using qoowork server.',
          ...
          'If the system prompt includes a qoowork media reference mapping, use mapped file paths or URLs in image/images/firstFrame/referenceImages/video/videos/media arguments and never pass @ media tokens as tool argument values.',
        ...
        async execute(id: string, params: unknown, _signal?: AbortSignal, onUpdate?: (result: { content: Array<{type: string; text: string}>; details?: Record<string, unknown> }) => void) {
          ...
          if (action === 'status') {
            ...
            try {
              api.logger.info(`[qoowork-media-generation] video status polling started: toolCallId=${id} taskId=${taskId}`);
              const startedAt = Date.now();
              ...
              while (true) {
                ...
                if (elapsed >= VIDEO_POLL_TIMEOUT_MS) {
                  api.logger.info(`[qoowork-media-generation] video status poll timeout: toolCallId=${id} taskId=${taskId} elapsedMs=${elapsed} pollCount=${pollCount}`);
                  return {
                ...
                try {
                  ...
                  if (currentStatus && isTerminalStatus(currentStatus)) {
                    api.logger.info(`[qoowork-media-generation] video status poll complete: toolCallId=${id} taskId=${taskId} status=${currentStatus} elapsedMs=${Date.now() - startedAt} pollCount=${pollCount}`);
                    return {
                      ...statusResult,
                      details: {
                  ...
                  if (pollCount % 6 === 0) {
                    ...
                    api.logger.info(`[qoowork-media-generation] video status poll progress: toolCallId=${id} taskId=${taskId} pollCount=${pollCount} progress=${progress} elapsedMs=${Date.now() - startedAt}`);
                  }
                } catch (pollError) {
                  const pollMsg = pollError instanceof Error ? pollError.message : String(pollError);
                  api.logger.info(`[qoowork-media-generation] video status poll error (will retry): toolCallId=${id} taskId=${taskId} pollCount=${pollCount} error=${pollMsg}`);
                }
              }
            } catch (error) {
              const message = error instanceof Error ? error.message : String(error);
              api.logger.info(`[qoowork-media-generation] video status failed: toolCallId=${id} error=${message}`);
          ...
          try {
            api.logger.info(`[qoowork-media-generation] video tool (${action}) started: toolCallId=${id} args=${JSON.stringify(sanitizeArgsForLog(args))}`);
            const startedAt = Date.now();
            const result = await callMediaBridge(config, {
              tool: 'qoowork_video_generate',
              args,
            ...
            api.logger.info(`[qoowork-media-generation] video tool (${action}) completed: toolCallId=${id} elapsedMs=${Date.now() - startedAt} isError=${result.isError === true}`);
            return result;
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            api.logger.info(`[qoowork-media-generation] video tool (${action}) failed: toolCallId=${id} error=${message}`);
    ...
    api.logger.info('[qoowork-media-generation] registered qoowork_image_generate and qoowork_video_generate tools.');
  },
};
