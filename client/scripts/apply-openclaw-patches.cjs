...
/**
 * Apply version-specific qoowork patches to the openclaw source tree.
 *
...
// This removes stale patches left by a different qoowork branch that may have
...
for (const patchFile of patchFiles) {
  ...
  if (needsNormalize) {
    patchPath = path.join(os.tmpdir(), `qoowork-patch-${patchFile}`);
