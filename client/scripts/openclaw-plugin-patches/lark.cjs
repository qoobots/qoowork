...
// Lightweight setup entry for deferred loading (patched by qoowork).
...
function patchFilenameEncoding(larkPluginDir, log) {
  ...
  const replacement = `fileName = decodeURIComponent(match[1].trim());
                // Patched by qoowork: fix Latin-1 garbled UTF-8 filenames from Feishu API
  ...
  if (fnIdx !== -1) {
    const helperFn = `// Patched by qoowork: detect and fix Latin-1 garbled UTF-8 filenames.
