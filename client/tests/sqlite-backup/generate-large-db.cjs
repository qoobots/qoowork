...
const DEFAULTS = {
  db: path.resolve(process.cwd(), 'qoowork.sqlite'),
  sessions: 20,
...
Examples:
  node tests/sqlite-backup/generate-large-db.cjs --db ~/tmp/qoowork.sqlite
  node tests/sqlite-backup/generate-large-db.cjs --db ./tmp/qoowork.sqlite --sessions 50 --messages-per-session 10000 --payload-kb 16
`);
}
...
function createPayloadBytes(targetBytes) {
  const line = 'The quick brown qoowork benchmarks SQLite backup throughput with wide message payloads.\n';
...
function main() {
  ...
  const tempSqlPath = path.join(
    os.tmpdir(),
    `qoowork-sqlite-backup-seed-${process.pid}-${Date.now()}.sql`,
  );
