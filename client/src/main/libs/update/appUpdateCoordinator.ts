...
const APP_UPDATE_TEST_CURRENT_VERSION_ENV = 'qoowork_UPDATE_CURRENT_VERSION';
...
export class AppUpdateCoordinator {
  ...
  private isCachedInstallerForSource(filename: string, source: AppUpdateSource | null): boolean {
    if (!filename.startsWith('qoowork-update-')) {
      return false;
    }
    if (source == null) {
      return true;
    }
    if (filename.startsWith(`qoowork-update-${source}-`)) {
      return true;
    }
    return /^qoowork-update-\d+/.test(filename);
  }
