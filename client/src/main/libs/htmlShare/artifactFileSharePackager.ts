...
const MARKDOWN_ASSET_PREFIX = '_QOOWORK_assets/';
const MARKDOWN_MANIFEST_FILE = '_QOOWORK_share_manifest.json';
...
async function writeSingleFileZip(file: LoadedArtifactFile): Promise<{ archivePath: string; sourceSha256: string }> {
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'qoowork-artifact-share-'));
  try {
