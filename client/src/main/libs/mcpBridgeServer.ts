...
export class McpBridgeServer {
   ...
   * The callback should call qoowork-server and return the result.
   */
  ...
  private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    const authHeader = req.headers['x-mcp-bridge-secret'] || req.headers['x-ask-user-secret'] || req.headers['x-qoowork-media-secret'];
  ...
  private async handleAskUser(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    try {
      ...
      const userResponse = await new Promise<AskUserResponse>((resolve) => {
        ...
        // Notify qoowork to show the modal
        if (this.onAskUserCallback) {
