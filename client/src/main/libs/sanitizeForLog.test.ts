...
describe('sanitizeUrlForLog', () => {
  test('removes query values and fragments from valid URLs', () => {
    const result = sanitizeUrlForLog(
      'https://rlogs.qoobot.com/rlog.php?action=qoowork_app_started&log_Usid=user-1#result',
    );

    expect(result).toBe('https://rlogs.qoobot.com/rlog.php?[redacted]#[redacted]');
    expect(result).not.toContain('user-1');
    expect(result).not.toContain('qoowork_app_started');
  });
