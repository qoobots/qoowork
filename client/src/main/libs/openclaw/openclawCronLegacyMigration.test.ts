...
describe('openclawCronLegacyMigration', () => {
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qoowork-openclaw-cron-migration-'));
  ...
  test('runs official doctor fix when jobs.json exists', async () => {
    ...
    expect(options.env.OPENCLAW_CONFIG_PATH).toBe(path.join(stateDir, '.qoowork-cron-doctor-openclaw.json'));
