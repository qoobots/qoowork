...
import type { DingTalkInstanceConfig, EmailMultiInstanceConfig, FeishuInstanceConfig, QoobotBeeChanConfig, NimInstanceConfig, PopoInstanceConfig, QQInstanceConfig, WecomInstanceConfig, WeixinOpenClawConfig } from '../../im/types';
...
const MANAGED_SKILL_ENTRY_OVERRIDES: Record<string, { enabled: boolean }> = {
  ...
  // qoowork configures MCP servers via openclaw.json mcp.servers field.
  ...
  // users are routed through qoowork's MCP layer instead.
  'mcporter': {
    enabled: false,
  },
};
...
const providerApiKeyEnvVar = (providerName: string): string => {
  ...
  return `QOOWORK_APIKEY_${envName}`;
};
...
const MANAGED_WEB_SEARCH_POLICY_PROMPT = [
  ...
  '- Only use the qoowork `web-search` skill when local command execution is available. Native channel sessions may deny `exec`, so prefer `browser` or `web_fetch` there.',
  ...
  '',
  'Do not claim you searched the web unless you actually used `browser`, `web_fetch`, or the qoowork `web-search` skill.',
].join('\n');
...
const MANAGED_BROWSER_POLICY_PROMPT = [
  '## Browser Policy',
  '',
  'qoowork does not support sandbox browser execution in this version.',
  '- For every `browser` tool call, set `target="host"` explicitly.',
  '- Do not use `target="sandbox"` or `target="node"` unless a future qoowork version explicitly enables it.',
 ...
 *
 * Example outputs:
 *   macOS:   ~/Library/Application Support/qoowork/SKILLs
 *   Windows: ~/AppData/Roaming/qoowork/SKILLs
 *   Linux:   ~/.config/qoowork/SKILLs
 */
...
const buildManagedSkillCreationPrompt = (skillsDirPath: string): string => [
  '## Skill Creation',
  '',
  'When the user asks you to create a new skill, you MUST place it under the qoowork skills directory:',
  '',
...
const PROVIDER_REGISTRY: Record<string, ProviderDescriptor> = {
  [ProviderName.qooworkServer]: {
    providerId: OpenClawProviderId.qooworkServer,
    ...
    resolveApiKey: () => {
      const proxyPort = getOpenClawTokenProxyPort();
      return proxyPort ? '${QOOWORK_PROXY_TOKEN}' : `\${${providerApiKeyEnvVar('server')}}`;
    },
  },
  ...
  [ProviderName.Copilot]: {
    providerId: OpenClawProviderId.qooworkCopilot,
    resolveApi: () => OpenClawApiConst.OpenAICompletions as OpenClawProviderApi,
    normalizeBaseUrl: stripChatCompletionsSuffix,
    ...
    resolveApiKey: () => '${QOOWORK_PROXY_TOKEN}',
  },
};

const DEFAULT_DESCRIPTOR: ProviderDescriptor = {
  providerId: OpenClawProviderId.Qoowork,
  resolveApi: ({ apiType, baseURL }) => mapApiTypeToOpenClawApi(apiType, undefined, baseURL),
  normalizeBaseUrl: stripChatCompletionsSuffix,
};
...
const resolveDescriptor = (
  ...
  return {
    ...DEFAULT_DESCRIPTOR,
    providerId: providerName || OpenClawProviderId.Qoowork,
  };
};
...
type OpenClawConfigSyncDeps = {
  ...
  getQoobotBeeChanConfig: () => QoobotBeeChanConfig | null;
...
export class OpenClawConfigSync {
  ...
  private readonly getQoobotBeeChanConfig: () => QoobotBeeChanConfig | null;
  ...
  constructor(deps: OpenClawConfigSyncDeps) {
    ...
    this.getQoobotBeeChanConfig = deps.getQoobotBeeChanConfig;
   ...
   * `.clobbered.<timestamp>` snapshot.  Because qoowork writes openclaw.json
...
loopDetection: MANAGED_TOOL_LOOP_DETECTION,
  ...
  sync(reason: string): OpenClawConfigSyncResult {
    ...
    if (apiResolution.config) {
      ...
      if (providerSelection.providerId === OpenClawProviderId.qooworkServer) {
      ...
      if (proxyPort) {
        ...
        const providerId = OpenClawProviderId.qooworkServer;
        ...
        if (serverModels.length > 0 || !allProvidersMap[providerId]) {
          ...
          const firstServerSel = buildProviderSelection({
            ...
            providerName: ProviderName.qooworkServer,
          ...
          const qooworkProviderConfig =
            allProvidersMap[providerId] ?? {
              ...firstServerSel.providerConfig,
              models: [] as typeof firstServerSel.providerConfig.models,
            };
          allProvidersMap[providerId] = qooworkProviderConfig;

          if (serverModels.length === 0) {
            upsertProviderModel(qooworkProviderConfig, firstServerSel.providerConfig.models[0]);
          } else {
            for (const sm of serverModels) {
              ...
              upsertProviderModel(qooworkProviderConfig, serverSel.providerConfig.models[0]);
            }
          }
        }
      }
    ...
    const hasMediaGenPlugin = isBundledPluginAvailable('qoowork-media-generation');
    ...
    const qoobotBeeChanConfig = this.getQoobotBeeChanConfig();
    ...
    const managedConfig: Record<string, unknown> = {
      ...
      ...((() => {
        ...
        const pluginEntries: Record<string, unknown> = {
          ...
          ...Object.fromEntries(
            preinstalledPlugins.map(plugin => {
              ...
              const pluginEnabled = (() => {
                ...
                if (pluginMatches(plugin, 'openclaw-qoobot-bee')) return !!(qoobotBeeChanConfig?.enabled && qoobotBeeChanConfig.clientId && qoobotBeeChanConfig.secret);
          ...
          ...(hasMediaGenPlugin ? { 'qoowork-media-generation': { enabled: true } } : {}),
          ...
          // Disable acpx (ACP agent runtime) — qoowork does not use ACP and
    ...
    if (hasAskUserPlugin && askUserCallbackUrl && managedConfig.plugins) {
      ...
      entries['ask-user-question'] = {
        ...
        config: {
          ...
          secret: '${QOOWORK_MCP_BRIDGE_SECRET}',
        },
      };
    }

    // Sync QooworkMediaGeneration plugin config — uses media callback endpoint
    const mediaCallbackUrl = this.getMediaCallbackUrl?.();
    if (hasMediaGenPlugin && mediaCallbackUrl && managedConfig.plugins) {
      const plugins = managedConfig.plugins as Record<string, unknown>;
      const entries = plugins.entries as Record<string, Record<string, unknown>>;
      entries['qoowork-media-generation'] = {
        enabled: true,
        config: {
    ...
    if (enabledTelegramInstances.length > 0) {
      ...
      for (let idx = 0; idx < enabledTelegramInstances.length; idx++) {
        ...
        const tokenVar = idx === 0 ? 'QOOWORK_TG_BOT_TOKEN' : `QOOWORK_TG_BOT_TOKEN_${idx}`;
        const webhookSecretVar = idx === 0 ? 'QOOWORK_TG_WEBHOOK_SECRET' : `QOOWORK_TG_WEBHOOK_SECRET_${idx}`;
    ...
    if (enabledDiscordInstances.length > 0) {
      ...
      for (let idx = 0; idx < enabledDiscordInstances.length; idx++) {
        ...
        const tokenVar = idx === 0 ? 'QOOWORK_DC_BOT_TOKEN' : `QOOWORK_DC_BOT_TOKEN_${idx}`;
        const account: Record<string, unknown> = {
          enabled: true,
          name: inst.instanceName,
          token: `\${${tokenVar}}`,
    ...
    if (enabledFeishuInstances.length > 0) {
      ...
      for (let idx = 0; idx < enabledFeishuInstances.length; idx++) {
        ...
        const secretVar =
          idx === 0 ? 'QOOWORK_FEISHU_APP_SECRET' : `QOOWORK_FEISHU_APP_SECRET_${idx}`;
    ...
    if (enabledDingTalkInstances.length > 0) {
      const buildDingTalkAccountConfig = (
        ...
        // gatewayBaseUrl were qoowork-specific and are not in the plugin schema.
        dmPolicy: inst.dmPolicy || 'open',
        allowFrom: (() => {
          const ids = inst.allowFrom?.length ? [...inst.allowFrom] : [];
          if (inst.dmPolicy === 'open' && !ids.includes('*')) ids.push('*');
      ...
      for (let idx = 0; idx < enabledDingTalkInstances.length; idx++) {
        ...
        const secretVar =
          idx === 0 ? 'QOOWORK_DINGTALK_CLIENT_SECRET' : `QOOWORK_DINGTALK_CLIENT_SECRET_${idx}`;
    ...
    if (enabledQQInstances.length > 0) {
      ...
      for (let idx = 0; idx < enabledQQInstances.length; idx++) {
        ...
        const secretVar =
          idx === 0 ? 'QOOWORK_QQ_CLIENT_SECRET' : `QOOWORK_QQ_CLIENT_SECRET_${idx}`;
    ...
    if (enabledWecomInstances.length > 0) {
      ...
      for (let idx = 0; idx < enabledWecomInstances.length; idx++) {
        ...
        const secretVar = idx === 0 ? 'QOOWORK_WECOM_SECRET' : `QOOWORK_WECOM_SECRET_${idx}`;
    ...
    if (enabledPopoInstances.length > 0) {
      ...
      for (let idx = 0; idx < enabledPopoInstances.length; idx++) {
        ...
        const secretVar = idx === 0 ? 'QOOWORK_POPO_APP_SECRET' : `QOOWORK_POPO_APP_SECRET_${idx}`;
        const account: Record<string, unknown> = {
          enabled: true,
          name: inst.instanceName,
        ...
        // Webhook-only fields
        if (!isWebSocket) {
          const tokenVar = idx === 0 ? 'QOOWORK_POPO_TOKEN' : `QOOWORK_POPO_TOKEN_${idx}`;
      ...
      if (enabledInstances.length > 0) {
        ...
        for (const inst of enabledInstances) {
          ...
          if (inst.transport === 'imap') {
            accountConfig.password = `\${QOOWORK_EMAIL_${envSuffix}_PASSWORD}`;
          ...
          if (inst.transport === 'ws') {
            accountConfig.apiKey = `\${QOOWORK_EMAIL_${envSuffix}_APIKEY}`;
          }
    ...
    if (configuredNimInstances.length > 0) {
      ...
      configuredNimInstances.forEach((inst, idx) => {
        const tokenEnvVar = idx === 0 ? 'QOOWORK_NIM_TOKEN' : `QOOWORK_NIM_TOKEN_${idx}`;
    ...
    // Sync QoobotBee OpenClaw channel config (via openclaw-netease-bee plugin)
    if (
      qoobotBeeChanConfig?.enabled &&
      qoobotBeeChanConfig.clientId &&
      qoobotBeeChanConfig.secret
    ) {
      managedConfig.channels = {
        ...((managedConfig.channels as Record<string, unknown>) || {}),
        'qoobot-bee': {
          enabled: true,
          clientId: qoobotBeeChanConfig.clientId,
          secret: qoobotBeeChanConfig.secret,
        },
      };
    }
  ...
  collectSecretEnvVars(): Record<string, string> {
    ...
    for (const [envSuffix, apiKey] of Object.entries(allApiKeys)) {
      console.info(`[OpenClawConfigSync] set secret env var QOOWORK_APIKEY_${envSuffix} for provider ${envSuffix}`);
      env[`QOOWORK_APIKEY_${envSuffix}`] = apiKey;
    }
    // Legacy fallback: keep QOOWORK_PROVIDER_API_KEY set to a stable value so stale
    ...
    env.QOOWORK_PROVIDER_API_KEY = 'legacy-unused';

    env.QOOWORK_PROXY_TOKEN = getCoworkOpenAICompatProxyToken() || 'unconfigured';

    // MCP Bridge Secret — always set so stale openclaw.json with
    // ${QOOWORK_MCP_BRIDGE_SECRET} placeholder doesn't crash the gateway.
    // Used by the ask-user-question plugin.
    env.QOOWORK_MCP_BRIDGE_SECRET = this.getMcpBridgeSecret?.() || 'unconfigured';
    ...
    for (let idx = 0; idx < enabledTelegram.length; idx++) {
      ...
      if (idx === 0) {
        env.QOOWORK_TG_BOT_TOKEN = inst.botToken;
        if (inst.webhookSecret) env.QOOWORK_TG_WEBHOOK_SECRET = inst.webhookSecret;
      } else {
        env[`QOOWORK_TG_BOT_TOKEN_${idx}`] = inst.botToken;
        if (inst.webhookSecret) env[`QOOWORK_TG_WEBHOOK_SECRET_${idx}`] = inst.webhookSecret;
      }
    }
    ...
    for (let idx = 0; idx < enabledDiscord.length; idx++) {
      if (idx === 0) {
        env.QOOWORK_DC_BOT_TOKEN = enabledDiscord[idx].botToken;
      } else {
        env[`QOOWORK_DC_BOT_TOKEN_${idx}`] = enabledDiscord[idx].botToken;
      }
    }
    ...
    for (let idx = 0; idx < enabledFeishu.length; idx++) {
      if (idx === 0) {
        env.QOOWORK_FEISHU_APP_SECRET = enabledFeishu[idx].appSecret;
      } else {
        env[`QOOWORK_FEISHU_APP_SECRET_${idx}`] = enabledFeishu[idx].appSecret;
      }
    }
    ...
    for (let idx = 0; idx < enabledDingTalk.length; idx++) {
      if (idx === 0) {
        env.QOOWORK_DINGTALK_CLIENT_SECRET = enabledDingTalk[idx].clientSecret;
      } else {
        env[`QOOWORK_DINGTALK_CLIENT_SECRET_${idx}`] = enabledDingTalk[idx].clientSecret;
      }
    }
    // Gateway token is shared (not per-instance)
    const gatewayToken = this.engineManager.getGatewayToken();
    if (gatewayToken) {
      env.QOOWORK_DINGTALK_GW_TOKEN = gatewayToken;
    }
    ...
    for (let idx = 0; idx < enabledQQ.length; idx++) {
      if (idx === 0) {
        env.QOOWORK_QQ_CLIENT_SECRET = enabledQQ[idx].appSecret;
      } else {
        env[`QOOWORK_QQ_CLIENT_SECRET_${idx}`] = enabledQQ[idx].appSecret;
      }
    }
    ...
    for (let idx = 0; idx < enabledWecom.length; idx++) {
      if (idx === 0) {
        env.QOOWORK_WECOM_SECRET = enabledWecom[idx].secret;
      } else {
        env[`QOOWORK_WECOM_SECRET_${idx}`] = enabledWecom[idx].secret;
      }
    }
    ...
    for (let idx = 0; idx < enabledPopo.length; idx++) {
      if (idx === 0) {
        env.QOOWORK_POPO_APP_SECRET = enabledPopo[idx].appSecret;
        if (enabledPopo[idx].token) {
          env.QOOWORK_POPO_TOKEN = enabledPopo[idx].token;
        } else {
          // Provide non-empty fallback so stale openclaw.json files that still
          // contain ${QOOWORK_POPO_TOKEN} from a previous webhook config
          // don't crash the gateway with MissingEnvVarError.
          env.QOOWORK_POPO_TOKEN = 'unconfigured';
        }
      } else {
        env[`QOOWORK_POPO_APP_SECRET_${idx}`] = enabledPopo[idx].appSecret;
        if (enabledPopo[idx].token) {
          env[`QOOWORK_POPO_TOKEN_${idx}`] = enabledPopo[idx].token;
        } else {
          env[`QOOWORK_POPO_TOKEN_${idx}`] = 'unconfigured';
        }
      }
    }
    ...
    if (emailConfig?.instances) {
      for (const inst of emailConfig.instances) {
        ...
        if (inst.transport === 'imap' && inst.password) {
          env[`QOOWORK_EMAIL_${envSuffix}_PASSWORD`] = inst.password;
        }

        if (inst.transport === 'ws' && inst.apiKey) {
          env[`QOOWORK_EMAIL_${envSuffix}_APIKEY`] = inst.apiKey;
        }
      }
    }
    ...
    for (let idx = 0; idx < nimInstances.length; idx++) {
      ...
      const key = idx === 0 ? 'QOOWORK_NIM_TOKEN' : `QOOWORK_NIM_TOKEN_${idx}`;
      env[key] = inst.token;
    }
  ...
  /**
   * Ensures exec-approvals.json under the qoowork-managed openclaw home has
  ...
  private syncManagedSessionStore(
    ...
    const shouldMigrateManagedModelRefs = !(
      selection.providerId === 'qoowork' && selection.sessionModelId === selection.legacyModelId
    );
    ...
    for (const [agentId, agent] of agentById.entries()) {
      ...
      for (const [sessionKey, rawEntry] of Object.entries(sessionStore)) {
        ...
        if (!/^agent:[^:]+:qoowork:/.test(sessionKey)) {
          continue;
        }
  ...
  /**
   * Resolve the qoowork SKILLs installation directory for OpenClaw's
   * `skills.load.extraDirs` configuration.
   *
   * Cross-platform paths (via Electron app.getPath('userData')):
   *   macOS:   ~/Library/Application Support/qoowork/SKILLs
   *   Windows: %APPDATA%/qoowork/SKILLs
   *   Linux:   ~/.config/qoowork/SKILLs
   */
  ...
  /**
   * Build per-skill `enabled` overrides from the qoowork SkillManager state,
   * so that skills disabled in the qoowork UI are also hidden from OpenClaw.
   */
  ...
  private buildBindings(): { bindings?: Array<Record<string, unknown>> } {
    ...
    const singleInstanceChannels: Array<{
      ...
      { getter: () => this.getQoobotBeeChanConfig(), channel: 'qoobot-bee', platform: 'qoobot-bee' },
  ...
  private writeMinimalConfig(configPath: string, _reason: string): OpenClawConfigSyncResult {
    ...
    // ${QOOWORK_APIKEY_X} placeholders for providers that are no longer
    ...
    if (currentContent) {
      try {
        ...
        // vars (${QOOWORK_TG_BOT_TOKEN} etc.) that are still injected when
        ...
        // ${QOOWORK_APIKEY_*} env vars that may no longer be set.
      } catch {
