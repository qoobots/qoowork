...
import { isqooworkQuotaExhaustedError } from '../../../common/coworkErrorClassify';
...
function extractQuotaErrorFromProxyErrorPayload(
  ...
  try {
    ...
    if (isErrorPayload && isqooworkQuotaExhaustedError(searchable)) {
      return {
    ...
    if (event === 'error' && isqooworkQuotaExhaustedError(payload)) {
