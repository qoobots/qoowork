...
import { getOpenClawTokenProxyPort } from '../openclaw/openclawTokenProxy';
...
function tryqooworkServerFallback(modelId?: string): MatchedProvider | null {
  ...
  const effectiveApiFormat = cachedMeta?.apiFormat
    ...
    : 'openai';
  console.debug('[ClaudeSettings] qoowork-server provider resolved:', {
    baseURL,
...
function resolveMatchedProvider(appConfig: AppConfig): { matched: MatchedProvider | null; error?: string } {
  ...
  if (!modelId) {
    ...
    if (!fallback) {
      const serverFallback = tryqooworkServerFallback(configuredModelId);
  ...
  // Handle qoowork-server provider: dynamically construct from auth tokens
  if (preferredProviderName === ProviderName.qooworkServer) {
    const serverMatch = tryqooworkServerFallback(modelId);
    if (serverMatch) {
  ...
  if (!providerEntry) {
    ...
    if (fallback) {
      ...
      const serverFallback = tryqooworkServerFallback(modelId);
      if (serverFallback) return { matched: serverFallback };
...
export function resolveCurrentApiConfig(target: OpenAICompatProxyTarget = 'local'): ApiConfigResolution {
  ...
  const effectiveApiKey = resolvedApiKey
    || (!providerRequiresApiKey(matched.providerName) ? 'sk-qoowork-local' : '');
  ...
  return {
    config: {
      apiKey: resolvedApiKey || 'qoowork-openai-compat',
...
export function resolveAllProviderApiKeys(): Record<string, string> {
  ...
  // qoowork-server token is now managed by the token proxy
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    ...
    result[envName] = apiKey || 'sk-qoowork-local';
  }
...
export function resolveAllEnabledProviderConfigs(): ProviderRawConfig[] {
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    if (!providerConfig?.enabled) continue;
    if (providerName === ProviderName.qooworkServer) continue;
    ...
    result.push({
      providerName,
      baseURL: effectiveBaseURL,
      apiKey: apiKey || 'sk-qoowork-local',
      apiType: effectiveApiFormat === 'anthropic' ? 'anthropic' : 'openai',
      authType: providerConfig.authType,
