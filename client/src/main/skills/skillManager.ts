...
function buildSkillEnv(): Record<string, string | undefined> {
  ...
  env.qoowork_ELECTRON_PATH = getElectronNodeRuntimePath();
...
const downloadGithubArchive = async (
    ...
    archiveUrlCandidates.push(
      {
        ...
        headers: { 'User-Agent': 'qoowork Skill Downloader' },
      },
      {
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'qoowork Skill Downloader',
...
export class SkillManager {
      ...
      if (fs.existsSync(localSource)) {
        ...
        if (stat.isFile()) {
          if (isZipFile(localSource)) {
            ...
            const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'qoowork-skill-zip-'));
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'qoowork-skill-npm-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'qoowork-skill-clawhub-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'qoowork-skill-'));
        cleanupPath = tempRoot;
  ...
  private async performUpgradeDownload(skillId: string, downloadUrl: string, existingSkillDir: string): Promise<{
    ...
    try {
      ...
      const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'qoowork-skill-upgrade-'));
      cleanupPath = tempRoot;
