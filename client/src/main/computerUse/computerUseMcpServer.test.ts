...
describe('resolveComputerUseRuntimePaths', () => {
  function writeRuntimeFixture(): {
    ...
    const runtimePackageRoot = path.join(rootDir, 'node_modules', '@qoowork', 'computer-use');
    const helperExePath = path.join(runtimePackageRoot, 'bin', 'windows', 'qoowork-computer-use.exe');
    ...
    fs.writeFileSync(path.join(rootDir, 'runtime.json'), `\uFEFF${JSON.stringify({
      ...
      clientModule: 'node_modules/@qoowork/computer-use/dist/windows/computer_use_client.js',
      helper: 'node_modules/@qoowork/computer-use/bin/windows/qoowork-computer-use.exe',
      runtimePackageRoot: 'node_modules/@qoowork/computer-use',
    })}`);
  ...
  test('configures the helper with qoowork branding', () => {
    writeRuntimeFixture();
    ...
    expect(config.strings?.usingComputer).toBe('qoowork正在使用你的电脑');
  ...
  test('reports Escape cancellation before renewing the helper turn', () => {
    ...
    expect(script).toContain("requireEnv('QOOWORK_COMPUTER_USE_HOME')");
    expect(script).toContain("requireEnv('QOOWORK_COMPUTER_USE_CLIENT_MODULE')");
    ...
    expect(script).toContain("const APPROVED_APP_META_KEY = 'x-qoowork-computer-use-approved-app'");
