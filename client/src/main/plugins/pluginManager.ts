...
} from '../libs/openclaw/openclawLocalExtensions';
...
export class PluginManager {
  ...
  async installPlugin(params: PluginInstallParams, onLog?: PluginInstallLogCallback): Promise<PluginInstallResult> {
    ...
    try {
      ...
      const stagingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qoowork-plugin-stage-'));
  ...
  private async packNpmPlugin(params: PluginInstallParams, onLog?: PluginInstallLogCallback): Promise<string> {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qoowork-plugin-'));
  ...
  private async packGitPlugin(params: PluginInstallParams, onLog?: PluginInstallLogCallback): Promise<string> {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qoowork-plugin-git-'));
   ...
   * local SQLite store. Discovers plugins installed outside of qoowork
