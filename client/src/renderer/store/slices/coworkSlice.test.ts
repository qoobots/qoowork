...
test('updateCurrentSessionModelOverride only patches the active session', () => {
  ...
  const activeState = coworkReducer(
    ...
    updateCurrentSessionModelOverride({
      sessionId: 'session-1',
      modelOverride: 'qoowork-server/qwen3.6-plus-YoudaoInner',
    }),
  );

  expect(activeState.currentSession?.modelOverride).toBe('qoowork-server/qwen3.6-plus-YoudaoInner');
  ...
  expect(ignoredState.currentSession?.modelOverride).toBe('qoowork-server/qwen3.6-plus-YoudaoInner');
});
...
test('updateToolUseMediaStatus preserves the highest media poll count', () => {
  const state = coworkReducer(undefined, setCurrentSession(makeSession({
    messages: [{
      id: 'tool-1',
      type: 'tool_use',
      content: 'Using tool: qoowork_video_generate',
      timestamp: 1,
      metadata: {
        toolName: 'qoowork_video_generate',
        toolUseId: 'call-1',
