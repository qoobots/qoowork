...
const getModelAnalyticsSource = (model: Model, selectorGroup: ModelSelectorChangeMeta['group']): string => {
  if (model.isServerModel || model.providerKey === ProviderName.qooworkServer || selectorGroup === ModelSelectorGroup.Server) {
    return 'package';
  }
  return 'custom';
};
...
const CoworkPromptInput = React.forwardRef<CoworkPromptInputRef, CoworkPromptInputProps>(
  ...
  const resolveSubmitModelAccessPrompt = useCallback((): ModelAccessPromptKind | null => {
    ...
    if (
      effectiveSelectedModel?.providerKey === ProviderName.qooworkServer
  ...
  const handleSelectKit = useCallback((kitId: string) => {
    ...
    reportPromptControl('kit_toggle', {
      ...
      kitSource: marketplaceKit ? 'qoowork-kits' : 'installed',
      targetEnabled: willSelect,
  ...
  const textareaClass = isCompact
    ? `w-full resize-none bg-transparent px-4 pb-1.5 text-sm leading-[var(--qoowork-leading-sm)] text-foreground placeholder:dark:text-foregroundSecondary/60 placeholder:text-secondary/60 focus:outline-none min-h-[${minHeight}px] max-h-[${maxHeight}px] ${hasActiveContext ? 'pt-1.5' : 'pt-2'}`
    : isLarge
    ? `w-full resize-none bg-transparent px-4 pb-2 text-foreground placeholder:dark:text-foregroundSecondary/60 placeholder:text-secondary/60 focus:outline-none min-h-[${minHeight}px] max-h-[${maxHeight}px] ${
      useHomeContextLayout
        ? `${hasActiveContext ? 'pt-2' : 'pt-3'} text-sm leading-[var(--qoowork-leading-prompt)]`
        : `${hasActiveContext ? 'pt-2' : 'pt-2.5'} text-[length:var(--qoowork-text-promptLarge)] leading-[var(--qoowork-leading-promptLarge)]`
    }`
  ...
  const renderMentionTextarea = ({
    ...
    placeholder: string;
      ...
      {value && hasMediaMentionHighlight && (
        ...
        style={{
          ...style,
          caretColor: 'var(--qoowork-text-primary)',
          }}
        />
      </div>
    );
