...
const NODE_DEPLOYMENT_PROJECT_DIRECTORY_STORAGE_PREFIX =
  'qoowork:node-deployment-project-directory:';
              ...
              : state === 'active'
                ...
                : state === 'done'
                  ...
                  : {
                      ...
                      segment:
                        'bg-[color-mix(in_srgb,var(--qoowork-foreground)_10%,var(--qoowork-background))] text-secondary',
                    };
          return (
            <div
              key={step}
...
const ArtifactPanel: React.FC<ArtifactPanelProps> = ({
  ...
  const handleOpenInBrowser = useCallback(() => {
    ...
    reportSelectedArtifactAction('open_in_browser', {
      openTarget: selectedArtifact.type === ArtifactTypeValue.Html ? 'QOOWORK_browser' : 'external_browser',
    });
...
(() => {
  ...
  if (window.__qooworkAnnotationCleanup) {
    window.__qooworkAnnotationCleanup();
  }

  const overlayRoot = document.createElement('div');
  overlayRoot.setAttribute('data-qoowork-annotation-ui', 'true');
  ...
  composer.setAttribute('data-qoowork-annotation-ui', 'true');
  ...
  const cleanup = () => {
    ...
    overlayRoot.remove();
    delete window.__qooworkAnnotationCleanup;
  };
  ...
  const isAnnotationUi = (target) => target?.closest?.('[data-qoowork-annotation-ui="true"]');
  ...
  window.__qooworkAnnotationCleanup = cleanup;
...
const BrowserTabContent: React.FC<BrowserTabContentProps> = ({
  ...
  const handleToggleAnnotation = useCallback(async () => {
    ...
    if (isAnnotating) {
      ...
      await webviewNode
        .executeJavaScript('window.__qooworkAnnotationCleanup?.()')
        .catch(() => undefined);
