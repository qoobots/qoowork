...
import { classifyErrorKey, isqooworkQuotaExhaustedError } from './coworkErrorClassify';
...
test('billing: qoowork free quota exhausted', () => {
  expect(classifyError('免费额度已用完，请升级套餐')).toBe('coworkErrorQuotaExhausted');
});

test('billing: qoowork daily free quota code exhausted', () => {
  expect(classifyError('{"error":{"message":"今日免费额度已用完","code":40200}}')).toBe('coworkErrorQuotaExhausted');
});

test('billing: qoowork free quota code exhausted', () => {
  expect(classifyError('{"error":{"message":"免费额度已用完，请升级套餐","code":40201}}')).toBe('coworkErrorQuotaExhausted');
});

test('billing: qoowork monthly credits exhausted', () => {
  expect(classifyError('本月积分已用完')).toBe('coworkErrorQuotaExhausted');
});

test('billing: qoowork monthly quota JSON payload', () => {
  expect(classifyError('{"type":"error","error":{"type":"proxy_error","message":"本月积分已用完","code":40202}}')).toBe('coworkErrorQuotaExhausted');
});

test('billing: detects qoowork quota exhausted for proxy helpers', () => {
  expect(isqooworkQuotaExhaustedError('monthly credits exhausted')).toBe(true);
  expect(isqooworkQuotaExhaustedError('Request failed with status 402')).toBe(false);
});
