...
describe('ProviderName constants', () => {
  test('contains expected provider keys', () => {
    ...
    expect(ProviderName.qooworkServer).toBe('qoowork-server');
  });
});
...
describe('ProviderRegistry', () => {
  test('providerIds returns 17 providers (no custom)', () => {
    ...
    expect(ids).not.toContain(ProviderName.qooworkServer);
  });
  ...
  test('resolveModelSupportsThinking preserves known reasoning model metadata', () => {
    ...
    expect(ProviderRegistry.resolveModelSupportsThinking('qoowork-server', 'glm-5.1-YoudaoInner', false)).toBe(false);
    expect(ProviderRegistry.resolveModelSupportsThinking('qoowork-server', 'glm-5.1-YoudaoInner', true)).toBe(true);
