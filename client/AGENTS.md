...
This root `AGENTS.md` is repository-level guidance for qoowork. Codex may also
...
qoowork is an Electron + React desktop application. Its core user-facing
...
`Cowork` is qoowork's product/session layer. The name is historical: it
started as a Claude Code-like in-house coding assistant, but in the current
codebase it means the qoowork layer that owns sessions, messages,
...
When changing OpenClaw-related behavior, first look for a qoowork-side
...
qoowork when the behavior is product-specific or can be expressed cleanly at
the integration boundary.

Use version-scoped OpenClaw patches only when the required behavior is inside
OpenClaw and there is no clean qoowork-side hook. Do not avoid a patch by
adding brittle or contorted qoowork workarounds.
...
- `src/main/libs/openclawConfigSync.ts`: renders qoowork state into
...
SQLite lives in Electron `app.getPath('userData')` as `qoowork.sqlite`.
...
Important paths:
- `%APPDATA%/qoowork/openclaw/state/openclaw.json` on Windows: generated
  OpenClaw config.
- `%APPDATA%/qoowork/openclaw/state/workspace-main`: main agent workspace.
- `%APPDATA%/qoowork/openclaw/state/workspace-{agentId}`: non-main agent
  workspaces.
...
- `AGENTS.md`: OpenClaw workspace instructions with a qoowork-managed section.
...
Main logs:
- Windows: `%APPDATA%/qoowork/logs/main-YYYY-MM-DD.log`
- macOS: `~/Library/Logs/qoowork/main-YYYY-MM-DD.log`
- Linux: `~/.config/qoowork/logs/main-YYYY-MM-DD.log`

Main log retention is 7 days. Max file size is 80 MB; overflow rotates to
`.old.log`.

OpenClaw gateway capture logs:
- Windows: `%APPDATA%/qoowork/openclaw/logs/gateway-YYYY-MM-DD.log`
- Retention is 3 days.
...
- Weixin and Qoobot Bee have single-instance style config.
